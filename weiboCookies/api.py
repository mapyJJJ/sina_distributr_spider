...
def get_conn():
    ...
    :return:
    """
    for name in { 'weibo': 'CookiesGenerator'}:
        print(name)
        if not hasattr(g, name):
            setattr(g, name + '_cookies', eval('CookiesRedisClient' + '(name="' + name + '",db='+str(REDIS_DB)+')'))
            setattr(g, name + '_account', eval('AccountRedisClient' + '(name="' + name + '",db='+str(REDIS_DB)+')'))
    return g
